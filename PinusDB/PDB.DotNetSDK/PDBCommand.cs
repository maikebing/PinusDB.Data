...
namespace PDB.DotNetSDK
{
    ...
    public class PDBCommand
    {
        ...
        public byte[] ExecuteInsertToBuffer(string sql)
        {
            byte[] recvPacke;
            lock (ConnObj)
            {
        ...
        internal string BuildSql(string sql, params PDBParameter[] parms)
        {
        ...
        public byte[] ExecuteQueryToBuffer(string sql, params PDBParameter[] parms)
        {
            string newSql = BuildSql(sql, parms);
            return ExecuteQueryToBuffer(newSql);
        }
        public byte[] ExecuteQueryToBuffer(uint reqMethod, string sql)
        {
            SuccessCount = 0;
            PDBErrorCode retVal = 0;
            ...
            lock (ConnObj)
            {
                byte[] sendPacket = MakeRequestPacket(reqMethod, sql);
                ConnObj.Request(sendPacket);

                recvPacket = ConnObj.Recv();
        ...
        internal void DecodePacket(byte[] resPacket, out PDBErrorCode retVal, DataTable resultTab)
        {
        ...
        internal List<object> GetRecord(uint fieldCnt, byte[] buf, ref int offset)
        {
            int valType = 0;
        ...
        internal List<(PDBType type, object obj,int len)> GetRecordWithLen(uint fieldCnt, byte[] buf, ref int offset)
        {
            int valType = 0;
            List<(PDBType type, object, int len)> objList = new List<(PDBType type, object, int len)>();
            ...
            for (uint i = 0; i < fieldCnt; i++)
            {
                ...
                valType = (int)IntTool.ReadRawVarint32(buf, ref offset);
                object obj = GetStoreData(buf, ref offset, valType);
                objList.Add(((PDBType) valType, obj,offset-s));
            }
            return objList;
        }
